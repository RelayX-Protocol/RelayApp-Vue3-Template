<template>
  <!-- Header container with dynamic safe area top padding -->
  <div class="api-header api-common-header" :style="style">
    <!-- Emit toggleMenu event when clicked -->
    <svg
      class="pointer icon-open-menu"
      viewBox="64 64 896 896"
      focusable="false"
      data-icon="menu-unfold"
      width="28px"
      height="28px"
      fill="#333333"
      aria-hidden="true"
      @click="toggleMenu" 
    >
      <path
        d="M408 442h480c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8H408c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8zm-8 204c0 4.4 3.6 8 8 8h480c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8H408c-4.4 0-8 3.6-8 8v56zm504-486H120c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8zm0 632H120c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8zM142.4 642.1L298.7 519a8.84 8.84 0 000-13.9L142.4 381.9c-5.8-4.6-14.4-.5-14.4 6.9v246.3a8.9 8.9 0 0014.4 7z"
      ></path>
    </svg>
    <div class="image-box">
      <img class="logo" :src="logo" />
      <img class="logoTitle" :src="logoTitle" />
    </div>
  </div>
</template>

<script setup lang="ts">
import {onMounted, computed } from 'vue'
import logo from '../assets/logo.png'
import logoTitle from '../assets/logo_title.png'
// Define component emits
import { defineEmits } from 'vue'
const emit = defineEmits(['toggleMenu'])
onMounted(() => {
 
})

// Function to emit toggleMenu event to parent component
const toggleMenu = ()=>{
   emit('toggleMenu')
}

const style = computed(() => ({
  background: '#ffffff',
  paddingTop: `${window.safeAreaTop + 20}px`,
}))
</script>


